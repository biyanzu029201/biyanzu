...
*Setup instructions for a Gitian build of BiYanZu using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BiYanZu
...
- [Building BiYanZu](#building-biyanzu)
...
Clone the git repositories for biyanzu and Gitian and then checkout the biyanzu version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/biyanzu/biyanzu.git
cd biyanzu
...
Follow the instructions in [doc/release-process.md](release-process.md) in the biyanzu repository
...
To build BiYanZu (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the biyanzu repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/biyanzu/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/biyanzu/biyanzu
...
For example:
```bash
URL=https://github.com/user/biyanzu.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit biyanzu=${COMMIT} --url biyanzu=${URL} ../biyanzu/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit biyanzu=${COMMIT} --url biyanzu=${URL} ../biyanzu/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit biyanzu=${COMMIT} --url biyanzu=${URL} ../biyanzu/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/biyanzu-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/biyanzu-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/biyanzu-build.assert
```
...
[biyanzu/gitian.sigs](https://github.com/biyanzu/gitian.sigs.phils/) repository, or if that's not possible create a pull
request. 
